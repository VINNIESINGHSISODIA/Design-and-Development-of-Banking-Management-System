...
bool customer_handler(int client_socket) {
    ...
    // Initialize semaphore using account number as unique identifier
    sem_id = init_semphore(customer.account_number, CUSTOMER_FILE);

    // Try to acquire lock for this customer account
    if (!lock(&sem_operation, sem_id)) {
/*        write_bytes = write(client_socket, "Another session is already active for this account. Please try again later.\n", 78);
  */
	    write_bytes = write(client_socket,
    "Another session is already active for this account. Please try again later.\n",
    strlen("Another session is already active for this account. Please try again later.\n"));

    	  return false;
    }
    printf("working with customer account %d\n", customer.account_number);
